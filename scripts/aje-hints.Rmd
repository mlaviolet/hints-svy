...
percent <- function(x, decimals = 2) round(100 * x, decimals)
...
The following code produces the frequency table of gender and educational attaintment on page 10.
...
As previously noted, the HINTS 5 Cycle 4 database contains a set of 50 replicate weights to compute accurate standard errors for statistical testing procedures. These replicate weights were created using a jackknife minus one replication method; thus when analyzing one iteration or group of HINTS data, the proper denominator degrees of freedom (ddf) is 49. To perform the Type 3 analysis of effects for the logistic regression model, we use the function `regTermTest()`. The `{broom}` package does not currently support tidying the resulting objects.
